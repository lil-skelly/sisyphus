...
void inject(pid_t pid) {
    ...
    printf("[*] Found suitable memory region at %ld\n", address);
...
int main() {
    ...
    if (pid < 0) {
        ...
        printf("[*] CHILD: In child proc with PID: %i\n", pid);
        ...
        printf("[*] CHILD: Dettached from PARENT\n");
        printf("[*] CHILD: Entering sleep\n");
        ...
        printf("[*] PARENT: Starting injection\n");
        inject(pid);
    }
    return 0;
}
